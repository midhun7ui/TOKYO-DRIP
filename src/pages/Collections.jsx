        ...
        const fetchProducts = async () => {
            try {
                ...
                const productsData = querySnapshot.docs.map(doc => ({
                    ...
                    // Filter client-side for "collections" OR undefined/null (legacy items)
                    .filter(p => {
                        const types = p.collectionTypes || [];
                        const type = p.collectionType;
                        // Show if explicitly in 'collections', OR if it has NO specific tags (legacy default)
                        const isCollection = types.includes('collections') || type === 'collections';
                        const hasNoTag = types.length === 0 && !type;

                        return isCollection || hasNoTag;
                    });
    ...
    return (
        <div className="shell">
            <div className="section-head">
                <h2>The Archive</h2>
                <p>Timeless pieces for the modern era.</p>
            </div>
                        ...
                        <Link to={`/product/${product._id}`} key={product._id} className="block group">
                            <article className="featured-card h-full">
                                <div className="card-image-container">
                                    ...
                                    <span className="absolute top-2 left-2 chip">{product.category || 'Collection'}</span>
                                </div>
